...
internal abstract class GodotSignalMapperBase<T> : ConnectableObservable<T>, IDisposable
{
    ...
    private readonly StringName _godotSignalName;
    private readonly CancellationTokenRegistration? _cancellationTokenRegistration;
    private Callable? _godotCallableOnNext;
    private int _connected;
    private int _disposed;

    private bool ShouldDisposeOnDisconnect => !_cancellationTokenRegistration.HasValue;

    protected GodotSignalMapperBase(GodotObject obj, StringName signalName, CancellationToken cancellationToken)
    {
        _godotObject = obj;
        _godotSignalName = signalName;
        if (cancellationToken.CanBeCanceled)
        {
            _cancellationTokenRegistration = cancellationToken.UnsafeRegister((state) => ((GodotSignalMapperBase<T>) state).Dispose(), this);
        }
    }
    ...
    public override IDisposable Connect()
    {
        ...
        {
            _godotCallableOnNext ??= OnNextAsGodotCallable();
            _godotObject.Connect(_godotSignalName, _godotCallableOnNext.Value);
        }
    ...
    private void Disconnect()
    {
        ...
        {
            _godotObject.Disconnect(_godotSignalName, _godotCallableOnNext!.Value);
        }
    }
...
internal class GodotSignalMapper<[MustBeVariant] T>(GodotObject obj, StringName signalName, CancellationToken cancellationToken) : GodotSignalMapperBase<T>(obj, signalName, cancellationToken)
{
    ...
    private void OnNextWithArgs(T a) => Subject.OnNext(a);
}
