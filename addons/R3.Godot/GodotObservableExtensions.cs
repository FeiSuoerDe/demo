...
internal sealed class Delta(Observable<Unit> source, GodotFrameProvider frameProvider) : Observable<double>
{
    ...
    {
        return source.Subscribe(new InnerDelta(observer, frameProvider));
    }

    private sealed class InnerDelta(Observer<double> observer, GodotFrameProvider frameProvider) : Observer<Unit>
    {
...
internal sealed class Delta<T>(Observable<T> source, GodotFrameProvider frameProvider) : Observable<(double Delta, T Item)>
{
    ...
    private sealed class InnerDelta(Observer<(double Delta, T Item)> observer, GodotFrameProvider frameProvider) : Observer<T>
    {
