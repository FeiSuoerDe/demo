...
public static class GodotUiNodeExtensions
{
    ...
    /// <summary>Observe Show with current `Value` on subscribe.</summary>
    public static Observable<double> OnValueChangedAsObservable(this global::Godot.Range range, CancellationToken cancellationToken = default)
    {
        return Observable.Create<double, (global::Godot.Range, CancellationToken)>((range, cancellationToken), static (observer, state) =>
        {
            var (s, cancellationToken) = state;
            observer.OnNext(s.Value);
            return Observable.FromEvent<global::Godot.Range.ValueChangedEventHandler, double>(h => new global::Godot.Range.ValueChangedEventHandler(h), h => s.ValueChanged += h, h => s.ValueChanged -= h, cancellationToken).Subscribe(observer);
        });
    }
