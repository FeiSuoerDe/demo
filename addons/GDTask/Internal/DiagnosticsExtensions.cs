    ...
    internal static class DiagnosticsExtensions
    {
        ...
        public static string CleanupAsyncStackTrace(this StackTrace? stackTrace)
        {
            ...
            for (int i = 0; i < stackTrace.FrameCount; i++)
            {
                ...
                if (mb != null && IgnoreLine(mb)) continue;
                ...
                // parameter
                sb.Append("(");
                sb.Append(string.Join(", ", mb?.GetParameters().Select(p => BeautifyType(p.ParameterType, true) + " " + p.Name) ?? Array.Empty<string>()));
                sb.Append(")");
		...
		[RequiresUnreferencedCode("Calls System.StateTypeEnum.GetMethods()")]
        ...
        private static string? BeautifyType(Type? t, bool shortName)
        {
            ...
            if (t.FullName?.StartsWith("System.ValueTuple") ?? false)
            {
                ...
                var elements = genericArgs.Select(t1 => BeautifyType(t1, true)).ToList();
            ...
            if (genericType != null)
                return TypeBeautifyRegex.Replace(genericType, "").Replace("GDTask.Triggers.", "")
                    .Replace("GDTask.Internal.", "").Replace("GDTask.", "") + "<" + innerFormat + ">";
            return null;
        }
        ...
        private static bool IgnoreLine(MethodBase methodInfo)
        {
            ...
            if (declareType != null && declareType.StartsWith("GDTask.CompilerServices"))
            {
                return true;
            }
            ...
            if (declareType != null && declareType.StartsWith("System.Threading.Tasks.Task"))
            {
                return true;
            }

            if (declareType != null && declareType.StartsWith("GDTask.GDTaskCompletionSourceCore"))
            {
                return true;
            }

            return declareType != null && declareType.StartsWith("GDTask.AwaiterActions");
        }
